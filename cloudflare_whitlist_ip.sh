...
for cfip in `cat /tmp/cf_ips`; do ufw allow proto tcp from $cfip to any port 80 comment 'Cloudflare HTTP IP'; ufw allow proto tcp from $cfip to any port 443 comment 'Cloudflare HTTPS IP'; done
